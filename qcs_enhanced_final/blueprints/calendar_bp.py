...
def api_events():
    ...
    if start and end:
        # Logic: An event is in the range if its start is before the view's end,
        # AND its end is after the view's start.
        conditions.append('''
            (e.event_date < ? AND ? < COALESCE(DATE(e.end_date, '+1 day'), DATE(e.event_date, '+1 day')))
        ''')
